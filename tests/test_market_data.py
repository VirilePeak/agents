...
class TestLiquidityGate:
    ...
    def test_passes_with_good_liquidity(self):
        ...
        gate = LiquidityGate(max_spread_bps=100, min_depth_1bp=100, min_depth_5bp=500)
    ...
    def test_blocks_low_depth(self):
        """Trade blocked when depth too low"""
        gate = LiquidityGate(min_depth_1bp=1000)  # $1000 min
        
        ob = OrderBook("0x123")
        ob.bids = [Mock(price=0.55, size=100)]  # Too small
        ob.asks = [Mock(price=0.56, size=100)]
        ob.last_update = time.time()
        
        allowed, reason = gate.check(ob)
        assert allowed is False
        assert "depth_1bp_too_low" in reason
    
    def test_blocks_stale_book(self):
        """Trade blocked when book is stale"""
        gate = LiquidityGate(max_book_age_s=5)
...
class TestExecutionDecisions:
    ...
    def test_skip_when_stale_quote(self):
        """Skip execution when quote is stale"""
        ...
        # Mock orderbook manager to return stale book
        engine.orderbook_mgr.update_from_snapshot("0x123", [(0.55, 1000)], [(0.57, 1000)])
    ...
    def test_skip_when_spread_too_wide(self):
        ...
        engine.orderbook_mgr.update_from_snapshot("0x123", [(0.50, 1000)], [(0.60, 1000)])
        ob = engine.orderbook_mgr.get_orderbook("0x123")
        ob.last_update = time.time()
        ...
        assert result.success is False
        assert "liquidity_gate" in result.error
